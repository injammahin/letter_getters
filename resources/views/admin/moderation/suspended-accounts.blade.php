...
@section('page_title', 'Suspended Accounts')
@section('page_subtitle', 'Review suspended child, parent, and adult accounts')
...
>
    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        <div>
            <h2 class="text-2xl font-semibold text-neutral-900">Suspended Accounts</h2>
            <p class="mt-1 text-sm text-neutral-500">
                Only suspended child, parent, and adult accounts are shown here.
            </p>
        </div>
    ...
    <div class="flex flex-wrap items-center gap-3">
        <a
            href="{{ route('admin.moderation.suspended-accounts') }}"
            class="rounded-full px-4 py-2 text-sm font-medium transition {{ $currentRole === 'all' ? 'bg-neutral-900 text-white' : 'border border-black/10 bg-white text-neutral-600 hover:border-neutral-400 hover:text-neutral-900' }}"
        >
            All
        </a>

        <a
            href="{{ route('admin.moderation.suspended-accounts', ['role' => 'child']) }}"
            class="rounded-full px-4 py-2 text-sm font-medium transition {{ $currentRole === 'child' ? 'bg-[#fff0f9] text-[#CB148B]' : 'border border-black/10 bg-white text-neutral-600 hover:border-[#CB148B] hover:text-[#CB148B]' }}"
        >
            Children
        </a>

        <a
            href="{{ route('admin.moderation.suspended-accounts', ['role' => 'parent']) }}"
            class="rounded-full px-4 py-2 text-sm font-medium transition {{ $currentRole === 'parent' ? 'bg-[#f5efff] text-[#620A88]' : 'border border-black/10 bg-white text-neutral-600 hover:border-[#620A88] hover:text-[#620A88]' }}"
        >
            Parents
        </a>

        <a
            href="{{ route('admin.moderation.suspended-accounts', ['role' => 'adult']) }}"
            class="rounded-full px-4 py-2 text-sm font-medium transition {{ $currentRole === 'adult' ? 'bg-[#eef7ff] text-sky-700' : 'border border-black/10 bg-white text-neutral-600 hover:border-sky-400 hover:text-sky-700' }}"
        >
            Adults
        </a>
    </div>
                    ...
                    @forelse($users as $index => $user)
                        <tr class="transition hover:bg-neutral-50/70">
                            ...
                            <td class="px-6 py-5 text-right">
                                <button
                                    type="button"
                                    @click="openActivateModal('{{ addslashes($user->name) }}', '{{ route('admin.moderation.users.activate', $user) }}')"
                                    class="inline-flex items-center gap-2 rounded-full border border-green-200 bg-green-50 px-4 py-2 text-sm font-medium text-green-700 transition hover:bg-green-100"
                                >
    ...
    >
        <div
            @click.outside="closeActivateModal()"
        ...
        >
            ...
            <h3 class="mt-4 text-xl font-semibold text-neutral-900">Activate Account</h3>

            <p class="mt-2 text-sm leading-7 text-neutral-500">
                Are you sure you want to activate
                <span class="font-medium text-neutral-900" x-text="activateUserName"></span>?
            </p>

            <form :action="activateAction" method="POST" class="mt-6 flex items-center justify-center gap-3">
                @csrf
                @method('PATCH')
                ...
                <button
                    type="submit"
                    class="rounded-full bg-green-600 px-5 py-2.5 text-sm font-medium text-white"
                >
                    Confirm Activate
                </button>
            </form>
